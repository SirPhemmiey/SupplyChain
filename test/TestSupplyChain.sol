...
contract TestSupplyChain {
    ...
    function beforeAll() public{
        ...
        seller = new Actor(supplyChain);
        buyer = new Actor(supplyChain);
        enemy = new Actor(supplyChain);
        ...
        seller.addItem("Brogues", itemPrice);
        seller.addItem("Socks", itemPrice);
    }
    ...
    function testFetchItems() public {
        ...
        (string memory receivedName, uint recievedSku, uint receivedPrice, uint receivedState, address receivedSeller, address receivedBuyer, bool receivedExist) = supplyChain.fetchItem(itemSku);

        Assert.equal(receivedName, itemName, "Should be name of added item");
    ...
    function testItemCanBePurchased() public  {
        ...
        (string memory receivedName, uint receivedSku, uint receivedPrice, uint receivedState, address receivedSeller, address receivedBuyer, bool receivedExist) = supplyChain.fetchItem(itemSku);
    ...
    function testItemCannotBeShipped() public {
        ...
        Assert.isFalse(result, "Item not sold cannot be shiped, even if added by seller.");
...
contract Actor {
    ...
    function addItem(string name, uint price) public {
        SupplyChain(supplyChain).addItem(name, price);
    }

    function buyItem(uint256 sku, uint256 amount) public returns(bool) {
        return address(supplyChain).call.value(amount)(abi.encodeWithSignature("buyItem(uint256)", sku));
    }

    function shipItem(uint256 sku) public returns(bool){
        return address(supplyChain).call(abi.encodeWithSignature("shipItem(uint256)", sku));
    }

    function receiveItem(uint256 sku) public returns(bool){
        return address(supplyChain).call(abi.encodeWithSignature("receiveItem(uint256)", sku));
    }
